...
def _request_produto_sefaz(gtin, raio, my_lat, my_lon, dias):
    ...
    resp.raise_for_status()  # Se status >= 400, gera HTTPError
...
def obter_produtos(request, gtin_list, raio, my_lat, my_lon, dias):
    ...
    # Vamos disparar as requisições em paralelo
    response_list = []
    ...
    # Agora processamos cada resposta como antes
    data_list = []
...
def consultar_combustivel(descricao, raio, my_lat, my_lon, dias):
    ...
    try:
        ...
        response.raise_for_status()  # Lança erro para status HTTP 4xx e 5xx
        data = response.json()
...
def obter_combustiveis(descricao, raio, my_lat, my_lon, dias):
    ...
    # Criar lista de produtos processados com distância calculada
    data_list = []
    ...
    # Criar DataFrame e selecionar os 3 estabelecimentos mais próximos
    df = pd.DataFrame(data_list)
    df = df.sort_values(by='DISTANCIA_KM').head(3)  # Seleciona os 3 mais próximos
    return df
...
#     Parâmetros:
#         descricao (str): Nome do combustível a ser pesquisado (ex: "Gasolina", "Etanol").
#         raio (int): Raio de busca em quilômetros.
#         my_lat (float): Latitude do usuário.
#         my_lon (float): Longitude do usuário.
#         dias (int): Número de dias para considerar nos preços.

#     Retorna:
#         dict | None: Resposta JSON da API se bem-sucedida, None se houver erro.
#     """
#     url = 'http://api.sefaz.al.gov.br/sfz-economiza-alagoas-api/api/public/produto/pesquisa'
#     payload = {
#         "produto": {"descricao": descricao},
...
#     headers = {
#         "Content-Type": "application/json",
#         "AppToken": "ad909a7a6f0d6a130941ae2a9706eec58c0bb65d"  # Substitua pelo token correto se necessário
#     }
...
#     Retorna:
#         pd.DataFrame: DataFrame contendo os 3 estabelecimentos mais próximos.
#     """
#     response = consultar_combustivel(descricao, raio, my_lat, my_lon, dias)
